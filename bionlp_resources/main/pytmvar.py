...
def annotation_func_tmVar(my_list, soft_dir_path, pwd, input_dir_path, output_dir_path):
    for i in range(len(my_list)):
        ...
        my_str = f.readline()
        try:
            os.mkdir(f'./temp_input_tmvar')
            os.mkdir(f'./temp_output_tmvar')
        except:
            pass
        ...
        if len(total_doc) == 0:
            ...
            f.write(str([]))
        try:
            shutil.rmtree('./temp_input_tmvar')
        except:
            pass
        try:
            shutil.rmtree('./temp_output_tmvar')
        except:
            pass

def pytmvar(soft_dir_path, input_dir_path, output_dir_path = './'):
    pwd = os.getcwd()
    try:
