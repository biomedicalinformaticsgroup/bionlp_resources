...
def pmc_oa_generation(path = './'):
    ...
    path = str(path + str(f"/pmc_oabulk_output_{fdate}"))
    ...
    for j in range(len(url)):
        ...
        for i in range(len(complete_links)):
            if complete_links[i] == 'https://ftp.ncbi.nlm.nih.gov/pub/pmc/oa_bulk/oa_noncomm/':
                ...
                try:
                    filename = wget.download(complete_links[i], out=f"{path}/zip_files")
                    print('\n')
    ...
    for file in os.listdir(f"{path}/zip_files"):   # get the list of files
        if '.tar.gz' in str(file): # if it is a zipfile, extract it
            # open file
            unzip_file = tarfile.open(str(f"{path}/zip_files/") + str(file))
            # extracting file
            unzip_file.extractall(f"{path}/unzip_files")
            unzip_file.close()

    print('Unzip process completed')

    command = subprocess.getstatusoutput(f"ls -lR {path}/unzip_files")
    ...
    for i in range(len(command)):
        f = open(f"{path}/pmcid_subdirect.txt", "a")
    ...
    pickle.dump( df_path, open( f"{path}/PMC_oa_full_text.p", "wb" ) )
